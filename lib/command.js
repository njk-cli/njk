...
export default async (args) => {
  ...
  program
    ...
    .version(
      chalk`
      {yellow njk}: ${await import('../package.json').version}
      {yellow nunjucks}: ${await import('nunjucks/package.json').version}
    `
    )
  ...
  if (program.watch) {
    ...
    watcher.on('change', (file) => {
      if (isInside(file, templates)) {
        ...
        log(chalk`Changed template {yellow ${relative(process.cwd(), file)}}`)
        api(files, opts)
      } else if (/\.njk|\.html|\.md|\.mdown|\.markdown/.test(extname(file))) {
        // if a file is changed render that file
        log(chalk`Changed {yellow ${relative(process.cwd(), file)}}`)
        api(file, opts)
      }
    })
  }
